...
class __SWCSS_PARSER {
  ...
  // We have a sub segment so we need to end the prevoius segments code
  // We go back to the place before our current subclasses name appears.
  ...
  createNestedCodeSegement(text, i) {
    // We are in a sub brace state, a brace within a brace.
    let idx = text.lastIndexOf("\n", i);
    let name = text.substring(idx, i).replace(/^\s+|\s+$/g, "");
    let spacer = " ";

    if (name[0] === "&") {
      // We have a child classname where 2 classes exist on the same div (eg. '.top.inner')
    ...
    this.currSegment.children.push(object); // Since it's a sub component we add it as a child
    this.currSegment = object;  // Make this the last object the newly created one
  }
...
class __SWCSS_CLASS_STRING_PROCESSOR { 
  ...
  static lineHeight(classArray, important) {
    return `line-height: ${
      0.25 + parseInt(classArray[1]) * 0.125
    }rem${important};`;
  }
  ...
  static width(classArray, important) {
    ...
    if (arr.length > 1) {
      return `width: ${
        (parseInt(arr[0]) / parseInt(arr[1])) * 100
      }%${important};`;
    } else {
  ...
  static handleMarginAndPadding(classArray, important, negativeValue, self) {
    ...
    if (val) {
      return `${val}: ${negativeValue}${
        parseInt(classArray[1]) * 0.125
      }rem${important};`;
    }
  }
  ...
  static borders(classArray, important, self) {
    ...
    if (classArray[1] === "rad") {
      ...
      return `${val}-width: ${
        parseInt(classArray[1] || 1) * 1
      }px${important}; ${val}-style: solid${important};`;
    }
  }
  ...
  /* Converts a string of class names into CSS, with the possible responsive sizes and defaults.
     Returns a types objects.
   */
  static process(str) {
    let types = {
      default: "",
    ...
    let r = "";
    items.forEach((e) => {
      // this.classesAlreadyAdded[e] = 1;

      if (e.length === 0) return;

      /* Flag negative values and clean up out class name. */
      let negativeValue = "";
      if (e[0] === "-") {
        negativeValue = "-";
      ...
      /* Split out any responsive sizes like "lg", "sm", "md" */
      let x = e.split(":");
      let prefix = "default";
      if (x.length > 1) {
        prefix = x[0];
      ...
      /* If there is an underscore set things up as !important */
      let important = "";
      if (e[0] === "_") {
        e = e.substring(1, e.length);
        important = " !important";
      ...
      /* Handle the one words. Get this out of the way first. */
      let tmp = null;
      if ((tmp = this.oneWorders(e, this))) {
        return (types[prefix] += tmp);
      }

      /* Genrate our classname array */
      let classArray = e.split("-");

      // Process first word before hyphen and check for match (bg in 'bg-blue-200')
      let key1 = classArray[0];
      if ((x = matcher[key1])) {
        let tmp = __SWCSS_CLASS_STRING_PROCESSOR[x](classArray, important, this) || "";
        if (tmp) {
          return (types[prefix] += tmp);
        }
      }
