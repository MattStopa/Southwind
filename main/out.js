...
(() => {
  ...
  // code/browserRender.js
  function browserRender_default() {
    ...
    function loadup() {
      ...
      Array.from(document.querySelectorAll("*")).map((i) => {
        i.className.toString().split(/\s+/).forEach((e) => {
          allClasses[e] = 1;
        });
      });
      ...
      let observer = new MutationObserver(function(mutations) {
        if (mutations[0].addedNodes[0] && mutations[0].addedNodes[0].nodeName === "#text") {
          return;
        }
  ...
  __publicField(SWCSS2, "classesAlreadyAdded", {});
  if (typeof window !== "undefined") {
    browserRender_default();
  }
})();
